...
## and re-apply the collection's past queries. Both stores are keyed by collection
## name (a project binds at most one database). A saved query is a plain dict:
...
## and marked when it carries find options.
